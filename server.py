...
class ChatServer(threading.Thread):
    ...
    def setup_server(self):
        ...
        self.server_socket_ssl = ssl_context.wrap_socket(
    ...
    def run_server(self):
        ...
        while True:
            ...
            client_thread = threading.Thread(
                target=self.handle_client, args=(client_socket_ssl,))
            client_thread.start()
            self.unauthenticated_clients[client_socket_ssl] = {
                'rsa_public_key': None,  'x25519_public_key': None}
    ...
    def handle_client(self, client_socket_ssl):
        ...
        while is_running:
            try:
                ...
                sockets = [client_data['socket']
                           for client_data in self.authenticated_clients.values()]
                if client_socket_ssl in sockets:
                    self.handle_authenticated_message(
                        message, client_socket_ssl)
                else:
                    self.handle_unauthenticated_message(
                        message, client_socket_ssl)
                ...
                if username is not None:
                    self.alert_all_users(
                        StatusCode.USER_LOGGED_OUT, f'{username}')

                logging.info("Client disconnected. Peer address: %s.",
                             client_socket_ssl.getpeername())
                is_running = False
    ...
    def handle_authenticated_message(self, message, client_socket_ssl):
        ...
        if handler:
            ...
            self.send_status_response(
                StatusCode.INVALID_MESSAGE_TYPE, client_socket_ssl)
            logging.info("Received an invalid message type: %s.", message_type)
    ...
    def handle_public_key_x25519(self, data, client_socket_ssl):
        ...
        self.unauthenticated_clients[client_socket_ssl].update(
            {'x25519_public_key': public_key})
        public_key_pem = public_key.public_bytes(
            Encoding.PEM, PublicFormat.SubjectPublicKeyInfo)
        peer_address = client_socket_ssl.getpeername()
        logging.info('Public key received from %s: %s.',
                     peer_address, public_key_pem)
    ...
    def handle_login(self, data, client_socket_ssl):
        ...
        if username in self.authenticated_clients:
            self.send_status_response(
                StatusCode.USERNAME_ALREADY_LOGGED_IN, client_socket_ssl)
            return
        ...
        if self.database.authenticate_user(username, password):
            ...
            self.send_status_response(
                StatusCode.INVALID_LOGIN, client_socket_ssl)
        ...
        if recipient in self.authenticated_clients:
            ...
            if token == self.authenticated_clients[sender]['token']:
                self.send_message(sender, recipient,
                                  message_b64, recipient_socket_ssl)
            else:
                logging.info("Invalid token from %s.", sender)
    ...
    def alert_all_users(self, status_code, message):
        for _, client_data in self.authenticated_clients.items():
            ...
            for recipient_username, recipient_client_data in self.authenticated_clients.items():
                if recipient_username != public_key_username:
                    ...
                    self.send_public_key(
                        public_key, public_key_username, recipient_socket_ssl)
...
if __name__ == '__main__':
    server = ChatServer(SERVER_HOST, SERVER_PORT, CERT_FILE, KEY_FILE)
    server.start()
